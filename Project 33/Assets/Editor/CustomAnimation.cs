...
public class LevelScriptEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        polygonMode = EditorGUILayout.Toggle("Animate Colliders", polygonMode);
        ((CustomAnimation)target).animateColliders = polygonMode;
        ((CustomAnimation)target).loop = EditorGUILayout.Toggle("Loop", ((CustomAnimation)target).loop);
        ((CustomAnimation)target).randomizeStartingFrame = EditorGUILayout.Toggle("Randomize Starting Frame", ((CustomAnimation)target).randomizeStartingFrame);
        if (polygonMode) EditorGUILayout.PropertyField(serializedObject.FindProperty("animationColliders"), true);
        ...
        if (polygonMode) EditorGUILayout.LabelField("Collider Generation", EditorStyles.boldLabel); // Label to show that collider generation settings begins below.
        if (polygonMode) minTransparency = EditorGUILayout.Slider("Minimum Transparency", minTransparency, 0, 1); // Slider to set how low transparency has to be before counting as transparent (0 - 1).

        if (polygonMode)
            if (GUILayout.Button("Generate/Update PolygonCollider2D for each frame.")) // Button to generate colliders.             
                generateColliders((CustomAnimation)target); // And if it's pressed, well, generate the colliders.
